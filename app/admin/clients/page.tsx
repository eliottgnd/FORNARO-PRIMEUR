    ...
    <div>
      ...
      <AnimateIn delay={0.1}>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 mb-6 md:mb-8">
          {[
            { label: 'Total clients',      valeur: clients.length },
            { label: 'Commandes moyennes', valeur: Math.round(clients.reduce((a, c) => a + c.commandes, 0) / clients.length) },
            { label: 'Panier moyen',       valeur: `${(clients.reduce((a, c) => a + c.total, 0) / clients.length).toFixed(2)}€` },
          ].map((s) => (
              ...
              <div key={c.id}>
                ...
                <div className="hidden md:grid grid-cols-[2fr_2fr_1fr_1fr] px-6 py-4 items-center hover:bg-creme/40 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-[14px] font-medium text-texte">{c.prenom} {c.nom}</p>
                  </div>
                  <p className="text-[13px] text-gris">{c.email}</p>
                  <p className="font-display text-[15px] text-vert">{c.commandes}</p>
                  <p className="font-display text-[15px] text-vert">{c.total.toFixed(2)}€</p>
                </div>
                ...
                <div className="md:hidden px-4 py-4 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[13px] font-medium text-texte">{c.prenom} {c.nom}</p>
                      <p className="text-[11px] text-gris mt-0.5">{c.email}</p>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="font-display text-[15px] text-vert">{c.total.toFixed(2)}€</p>
                    <p className="text-[11px] text-gris mt-0.5">{c.commandes} cmd</p>
                  </div>
                </div>
